...
void trainNetwork(int setSize, double *trainingInputs, double *trainingOutputs)
{
	...
	for(int e = 0; e < epochs; e++)
	{
		...
		for(int i = 0; i < setSize; i++)
		{
			...
			updateLayer(outputAct, outputWeights, outputBiases, deltaOutput, nbOutputNodes, nbHiddenNodes, lr);
			...
			updateLayer(hiddenAct, hiddenWeights, hiddenBiases, deltaHidden, nbHiddenNodes, nbInputNodes, lr);
