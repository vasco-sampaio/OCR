...
//double dSigmoid(double x) { return x * (1.0 - x) * 5; }
...
// Return the result of a function applied on 2 matrices of same size
matrix_t dualMatFunc(matrix_t a, matrix_t b, double (*fct)(double, double))
{
	if(a.rows != b.rows || a.cols != b.cols)
	{
		errx(1, "Cant apply func to these matrices");
	}
	...
	for(int i = 0; i < a.rows; i++)
	{
		...
		{
			setMatVal(res, i, j, fct(getMatVal(a, i, j), getMatVal(b, i ,j)));
		}
	}
	return res;
}
